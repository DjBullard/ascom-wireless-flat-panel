...
* The BLE radio now transmits at maximum output power (previously the SoftDevice default of 0 dBm) to improve range: +4 dBm on the nRF52832, +8 dBm on the nRF52840. Same rationale as the always-fast advertising — the panel only runs while actively in use, so the extra draw is a non-issue.
